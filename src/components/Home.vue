<template>
  <div>
    <div class="header">
      <h1 class="title"> Algorithmic Disobedience</h1>
      <h4 class="subtitle"> A workshop by <a href="http://www.tegabrain.com/">Tega Brain</a> & <a href="http://www.suryamattu.com/">Surya Mattu</a></h4>
    </div>
    <div class="container">
      <div class="section">
        <h2>
          What is disobedience in a world shaped by algorithms?
        </h2>
        <p>
Algorithmic disobedience calls for action in a world shaped by data and structured by algorithms. Algorithmic systems influence what news we see, who we date and how our neighbourhoods are policed. The accumulation of data has become paramount to the ways that power and resources are distributed and yet the logic of these systems are built on patterns drawn out of historic datasets, bringing the conditions of the past back to haunt the present.
</p><p>
If individuals are legible only as aggregates of data, as products of pattern and probability, how can we push back? This workshop looks to recent work by artists and journalists for tactics of algorithmic disobedience.
</p>
    </div>
    <div class="section">
      <h2>
        Goals of the workshop
      </h2>
      <p>
        We invite the playful, the defiant and the dissatisfied to explore the growing role of these systems in our lives and script small acts of audit, obfuscation or opting out.
        What are strategies that help us ask how we want these systems to be in our lives?
        <br><br>
         Read the <a href="https://drive.google.com/file/d/0B8pZRjIECCvPVUIxcmQtQmw3TjQ/view?usp=sharing">Algorithmic Disobedience Reader</a>  (5.6MB download)
      </p>
    </div>
    <div class="section">
    <h2>
    Upcoming dates:
    </h2>
    <p>
  <ul>
  <li>26th and 28th October: <a href="https://quantifiedselfonline.wordpress.com/events/">The Quantified Self Davies Forum</a>, University of San Fransisco.</li>
<li>11th November: <a href="https://theglassroom.org/schedule/events/algorithmic-disobedience-surya-mattu/">The Glass Room</a>, London.</li>
</ul>
    </p>
    </div>
    <Tactics/>
    </div>
  </div>
</template>

<script>
import Tactics from '@/components/Tactics';

export default {
  name: 'Home',
  components: { Tactics },
  data() {
    return {
    };
  },
};
</script>



<style lang="scss">

</style>
